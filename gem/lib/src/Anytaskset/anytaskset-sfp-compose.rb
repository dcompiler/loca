...
# thread subset within a given thread range
#
# Different from anytaskset-fp-compose.rb
# this tool counts the footprint of the 
# shared portion as only a part.
# For example, if two threads A and B
# access data d, only half of d is counted
# in A's footprint.
#
# example run:
#
# anytaskset-sfp-compose.rb -m [max_threads] -l [lowest_pillar] -p [pillar_step] -f [sfp_file] -g [sharing_graph.sg] --group=1,2 --range=1,2,3,4
#
# e.g. ./anytaskset-sfp-compose.rb -m 8 -l 12 -p 2 -f fp.out -g sg.out --group=1,2 --range=1,2,3,4
#
# Written by Hao Luo
#
#
...
OptionParser.new do |opts|
  ...
  opts.on('--range x,y,z', Array, "The set of all threads sharing the cache") do |f|
    options[:thread_range] = f.map {|x| x.to_i }
  end
end.parse!
...
class Composer
  ...
  # bitmap, e.g. integer
  # ==========================================
  def sfp_partial(thread_group, sharing_range)
    ...
    (0..@sgraph.size-1).each do |j|
      ...
      @sgraph[j].keys.each do |g|
        ...
        if Composer.has_intersection?(thread_group, g)
          part = Composer.count_ones(thread_group & g & sharing_range).to_f / Composer.count_ones(sharing_range & g).to_f
          porp[j][i] += @sgraph[j][g].to_f / @weight_map[j][i].to_f * part
        end
      end
  ...
  def print_partial_sfp(thread_group, group_range)

    ret = sfp_partial(thread_group, group_range)
    (0..@curve_length-1).each do |i|
...
group_range = Composer.array_to_bitmap(options[:thread_range])
c.print_partial_sfp(group_bitmap, group_range)
